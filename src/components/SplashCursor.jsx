...
// nendega, mitte juhusliku vikerkaarega.
const SPLASH_PALETTE = [
  { r: 0.18, g: 0.58, b: 0.72 },
  { r: 0.7, g: 0.23, b: 0.53 },
  { r: 0.94, g: 0.55, b: 0.11 },
  { r: 0.95, g: 0.76, b: 0.19 },
  { r: 0.34, g: 0.49, b: 0.2 },
];
...
export default function SplashCursor({
  ...
  useEffect(() => {
    ...
    class Material {
      ...
      setKeywords(keywords) {
        let hash = 0;
        for (let i = 0; i < (keywords?.length || 0); i++) hash += hashCode(keywords[i]);
    ...
    function generateColor() {
      const base = SPLASH_PALETTE[Math.floor(Math.random() * SPLASH_PALETTE.length)];
      return {
        r: base.r * colorScale,
        g: base.g * colorScale,
        b: base.b * colorScale,
    ...
    const onMouseMoveFirst = e => {
      const p = pointers[0];
      const x = scaleByPixelRatio(e.clientX);
      const y = scaleByPixelRatio(e.clientY);
      if (!raf && running && document.visibilityState === "visible") {
    ...
    document.body.addEventListener("mousemove", onMouseMoveFirst, {
      passive: true
    });
  ...
  return <div style={{
    ...
    height: "100%"
      ...
      <canvas ref={canvasRef} id="fluid" style={{
      width: "100vw",
      height: "100vh",
      display: "block",
