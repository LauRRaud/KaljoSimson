...
test("mobile header keeps navigation on one responsive touchable row", () => {
  ...
  const mobileNavRule = /@media \(max-width:\s*760px\)\s*\{[\s\S]*?\.site-nav\s*\{[\s\S]*?display:\s*flex;[\s\S]*?gap:\s*26px;[\s\S]*?width:\s*100%;[\s\S]*?max-width:\s*100vw;[\s\S]*?padding:\s*18px 28px;[\s\S]*?border-radius:\s*999px;/;
  const narrowNavRule = /@media \(max-width:\s*430px\)\s*\{[\s\S]*?\.site-nav\s*\{[\s\S]*?width:\s*100%;[\s\S]*?max-width:\s*100vw;[\s\S]*?flex-wrap:\s*nowrap;[\s\S]*?gap:\s*14px;[\s\S]*?padding:\s*14px 18px;[\s\S]*?overflow:\s*visible;[\s\S]*?border-radius:\s*999px;/;
  const narrowLinksRule = /@media \(max-width:\s*430px\)\s*\{[\s\S]*?\.site-nav__links\s*\{[\s\S]*?flex:\s*1 1 auto;[\s\S]*?gap:\s*14px;/;
  const narrowControlsRule = /@media \(max-width:\s*430px\)\s*\{[\s\S]*?\.site-nav__controls\s*\{[\s\S]*?gap:\s*14px;[\s\S]*?padding-left:\s*14px;[\s\S]*?pointer-events:\s*auto;/;
