...
test("public pages expose the saved frame preset for CSS", () => {
  ...
  assert.match(framePresetHydrator, /document\.querySelector\("\.page-shell"\)\?\.setAttribute\("data-frame-preset", nextPreset\)/);
  assert.match(css, /\.page-shell\[data-frame-preset="gold"\]\s*\{/);
